    ...
    async Create(event, menudata) {
        ...
        for (var i = 0; i < menudata.items.length; i++) {
            ...
            if (menudata.items[i]['icon']) {
                item.innerHTML += `<div class="sidebar-icon">${await app.getSvgIcon(
                    menudata.items[i]['icon']
                )}</div>`;
            }
    ...
    methods: {
        ...
        getHTMLStyle() {
            switch (this.cfg.visual.window_transparency) {
                ...
                default:
                    ...
                    document
                        .querySelector('body')
                        .classList.remove('notransparency');
                    break;
                case 'disabled':
                    document.querySelector('html').style.background = '#222';
                    document
                        .querySelector('body')
                        .classList.add('notransparency');
        ...
        async addSelectedToPlaylist(playlist_id) {
            ...
            for (let i = 0; i < self.selectedMediaItems.length; i++) {
                if (
                    self.selectedMediaItems[i].kind == 'song' ||
                    ...
                    (self.selectedMediaItems[i].kind == 'album' ||
                        self.selectedMediaItems[i].kind == 'albums') &&
                    self.selectedMediaItems[i].isLibrary != true
                ) {
                    self.selectedMediaItems[i].kind = 'albums';
                    let res = await self.mk.api.albumRelationship(
                        self.selectedMediaItems[i].id,
                        'tracks'
                    );
                    ...
                    self.selectedMediaItems[i].kind == 'library-song' ||
                    ...
                    self.selectedMediaItems[i].kind = 'library-albums';
                    let res = await self.mk.api.library.albumRelationship(
                        self.selectedMediaItems[i].id,
                        'tracks'
                    );
                    let ids = res.map(function (i) {
            ...
            this.mk.api.library
                .appendTracksToPlaylist(playlist_id, pl_items)
                .then(() => {
        ...
        async init() {
            ...
            // Set profile name
            this.chrome.userinfo = await this.mkapi(
                'personalSocialProfile',
                false,
                ''
            );
            // API Fallback
            ...
            if (localStorage.getItem('librarySongs') != null) {
                this.library.songs.listing = JSON.parse(
                    localStorage.getItem('librarySongs')
                );
                this.library.songs.displayListing = this.library.songs.listing;
            }
            if (localStorage.getItem('libraryAlbums') != null) {
                this.library.albums.listing = JSON.parse(
                    localStorage.getItem('libraryAlbums')
                );
                this.library.albums.displayListing =
                    this.library.albums.listing;
            }

            window.onbeforeunload = function (e) {
                window.localStorage.setItem(
                    'currentTrack',
                    JSON.stringify(app.mk.nowPlayingItem)
                );
                window.localStorage.setItem(
                    'currentTime',
                ...
                window.localStorage.setItem(
                    'currentQueue',
                    JSON.stringify(app.mk.queue.items)
                );
            };
                                ...
                                if (queue != null) {
                                    ...
                                    if (queue && queue.length > 0) {
                                        let ids = queue.map((e) =>
                                            e.playParams
                                                ? e.playParams.id
                                                : e.attributes.playParams
                                                ? e.attributes.playParams.id
                                                : ''
                                        );
                                        let i = 0;
                                        if (ids.length > 0) {
            ...
            MusicKit.getInstance().videoContainerElement =
                document.getElementById('apple-music-video-player');

            ipcRenderer.on('SoundCheckTag', (event, tag) => {
                let replaygain = self.parseSCTagToRG(tag);
                try {
                    CiderAudio.audioNodes.gainNode.gain.value = Math.min(
                        Math.pow(10, replaygain.gain / 20),
                        1 / replaygain.peak
                    );
                } catch (e) {}
            });
            ...
            this.mk.addEventListener(
                ...
                (a) => {
                    ...
                    self.playerLCD.playbackDuration =
                        self.mk.currentPlaybackTime;
                }
            );
            ...
            this.mk.addEventListener(
                ...
                (a) => {
                                ...
                                app.mk.api
                                    .song(
                                        app.mk.nowPlayingItem._songId ??
                                            app.mk.nowPlayingItem.relationships
                                                .catalog.data[0].id
                                    )
                                    .then((response) => {
                                        previewURL =
                    ...
                    let type =
                        self.mk.nowPlayingItem != null
                            ? self.mk.nowPlayingItem['type'] ?? ''
                            : '';
        ...
        select_removeMediaItem(id) {
            this.selectedMediaItems
                ...
                .forEach((item) => {
                    this.selectedMediaItems.splice(
                        this.selectedMediaItems.indexOf(item),
                        1
                    );
                });
        },
        ...
        async refreshPlaylists() {
            let self = this;
            this.apiCall(
                'https://api.music.apple.com/v1/me/library/playlist-folders/p.playlistsroot/children/',
                (res) => {
        ...
        sortPlaylists() {
            this.playlists.listing.sort((a, b) => {
                if (
                    a.type === 'library-playlist-folders' &&
                    b.type !== 'library-playlist-folders'
                ) {
                    return -1;
                } else if (
                    a.type !== 'library-playlist-folders' &&
        ...
        deletePlaylist(id) {
            ...
            if (confirm(`Are you sure you want to delete this playlist?`)) {
                app.mk.api.library.deletePlaylist(id).then((res) => {
                    ...
                    let found = self.playlists.listing.find(
                        (item) => item.id == id
                    );
                    if (found) {
                        self.playlists.listing.splice(
                            self.playlists.listing.indexOf(found),
                            1
                        );
                    }
                });
        ...
        async showArtistView(artist, title, view) {
            let response = await this.mk.api.artistView(
                artist,
                view,
                {},
                { view: view, includeResponseMeta: !0 }
            );
            await this.showCollection(response, title, 'artists');
        },
        async showRecordLabelView(label, title, view) {
            let response = await this.mk.api.recordLabelView(
                label,
                view,
                {},
                { view: view, includeResponseMeta: !0 }
        ...
        async showSearchView(term, group, title) {
            let response = await this.mk.api.search(
                term,
                {
                    platform: 'web',
                    groups: group,
        ...
        async getPlaylistContinuous(response, transient = false) {
            ...
            function getPlaylistTracks(next) {
                app.apiCall(app.musicBaseUrl + next, (res) => {
                    ...
                    self.showingPlaylist.relationships.tracks.data =
                        self.showingPlaylist.relationships.tracks.data.concat(
                            res.data
                        );
                    if (res.next) {
        ...
        async getPlaylistFromID(id, transient = false) {
            ...
            const params = {
                ...
                'fields[playlists]':
                    'curatorName,playlistType,name,artwork,url',
        ...
        progressBarStyle() {
            ...
            return {
                background:
                    'linear-gradient(to right, var(--keyColor) 0%, var(--keyColor) ' +
                    value +
                    '%, #333 ' +
                    value +
                    '%, #333 100%)'
        ...
        routeView(item) {
            let kind = item.attributes.playParams
                ? item.attributes.playParams.kind ?? item.type ?? ''
                : item.type ?? '';
            let id = item.attributes.playParams
                ? item.attributes.playParams.id ?? item.id ?? ''
                : item.id ?? '';
            let isLibrary = item.attributes.playParams
            ...
            if (true) {
                ...
                if (kind.toString().includes('apple-curator')) {
                    ...
                    kind.toString().includes('record-label') ||
                    kind.toString().includes('curator')
                ) {
        ...
        prevButton() {
            if (
                !app.prevButtonBackIndicator &&
                app.mk.nowPlayingItem &&
                app.mk.currentPlaybackTime > 2
            ) {
        ...
        async getNowPlayingItemDetailed(target) {
            let u = await app.mkapi(
                app.mk.nowPlayingItem.playParams.kind,
                app.mk.nowPlayingItem.songId == -1,
                app.mk.nowPlayingItem.songId != -1
                    ? app.mk.nowPlayingItem.songId
                    : app.mk.nowPlayingItem['id'],
        ...
        async searchAndNavigate(item, target) {
            ...
            switch (target) {
                case 'artist':
                    ...
                    try {
                        if (
                            ...
                            !item.relationships.artists.data[0].type.includes(
                                'library'
                            )
                        ) {
                            if (
                                item.relationships.artists.data[0].type ===
                                    'artist' ||
                                item.relationships.artists.data[0].type ===
                                    'artists'
                            ) {
                        ...
                        if (artistId == '') {
                            const url =
                                item.relationships.catalog.data[0].attributes
                                    .artistUrl;
                            artistId = url.substring(url.lastIndexOf('/') + 1);
                            if (artistId.includes('viewCollaboration')) {
                                artistId = artistId.substring(
                                    artistId.lastIndexOf('ids=') + 4,
                                    artistId.lastIndexOf('-')
                                );
                            }
                        }
                    } catch (_) {}
                    ...
                    if (artistId == '') {
                        let artistQuery = await app.mk.api.search(
                ...
                case 'album':
                    ...
                    try {
                        if (
                            ...
                            !item.relationships.albums.data[0].type.includes(
                                'library'
                            )
                        ) {
                            if (
                                item.relationships.albums.data[0].type ===
                                    'album' ||
                                item.relationships.albums.data[0].type ===
                                    'albums'
                            ) {
                    ...
                    if (albumId == '') {
                        try {
                            let albumQuery = await app.mk.api.search(
                ...
                case 'recordLabel':
                    ...
                    if (labelId == '') {
                        try {
                            let labelQuery = await app.mk.api.search(
                                item.attributes.recordLabel,
                                {
                                    limit: 1,
                                    types: 'record-labels'
                                }
        ...
        searchLibrarySongs() {
                    ...
                    if (self.library.songs.sortOrder == 'asc') {
                        if (
                            ...
                            return aa
                                .toString()
                                .toLowerCase()
                                .localeCompare(bb.toString().toLowerCase());
                        }
                        ...
                        if (
                            ...
                            return bb
                                .toString()
                                .toLowerCase()
                                .localeCompare(aa.toString().toLowerCase());
                        }
                    }
                });
            }
            ...
            if (this.library.songs.search == '') {
                ...
                this.library.songs.displayListing =
                    this.library.songs.listing.filter((item) => {
                        let itemName = item.attributes.name.toLowerCase();
                        let searchTerm =
                            this.library.songs.search.toLowerCase();
                        let artistName = '';
                        let albumName = '';
                        if (item.attributes.artistName != null) {
                            artistName =
                                item.attributes.artistName.toLowerCase();
                        }
        ...
        searchLibraryAlbums(index) {
            ...
            if (this.library.albums.search == '') {
                ...
                this.library.albums.displayListing =
                    this.library.albums.listing.filter((item) => {
                        let itemName = item.attributes.name.toLowerCase();
                        let searchTerm =
                            this.library.albums.search.toLowerCase();
                        let artistName = '';
                        let albumName = '';
                        if (item.attributes.artistName != null) {
                            artistName =
        ...
        searchLibraryArtists(index) {
            ...
            function sortArtists() {
                ...
                self.library.artists.displayListing.sort((a, b) => {
                    ...
                    if (self.library.artists.sortOrder[index] == 'asc') {
                        ...
                        self.library.artists.sortOrder[index] == 'desc'
                    ) {
                        if (
                            aa.toString().match(/^\d+$/) &&
                            bb.toString().match(/^\d+$/)
            ...
            if (this.library.artists.search == '') {
                this.library.artists.displayListing =
                    this.library.artists.listing;
                sortArtists();
            } else {
                this.library.artists.displayListing =
                    this.library.artists.listing.filter((item) => {
                        let itemName = item.attributes.name.toLowerCase();
                        let searchTerm =
                            this.library.artists.search.toLowerCase();
                        let artistName = '';
                        let albumName = '';
                        // if (item.attributes.artistName != null) {
        ...
        async mkapi(
            ...
            try {
                if (library) {
                    return await this.mk.api.library[method](
                        term,
                        params,
                        params2
                    );
        ...
        async getLibrarySongsFull(force = false) {
            ...
            this.library.downloadNotification.message =
                'Updating library songs...';
            ...
            function downloadChunk() {
                ...
                if (downloaded == null) {
                    app.mk.api.library
                        .songs('', params, { includeResponseMeta: !0 })
                        .then((response) => {
                            processChunk(response);
                        });
                } else {
                    downloaded
                        .next('', params, { includeResponseMeta: !0 })
                        .then((response) => {
                            processChunk(response);
                        });
                }
            ...
            function processChunk(response) {
                ...
                self.library.downloadNotification.message =
                    'Updating library songs...';
                ...
                if (
                    downloaded.meta.total > library.length ||
                    typeof downloaded.meta.next != 'undefined'
                ) {
                    console.log(
                        `downloading next chunk - ${library.length} songs so far`
                    );
                    downloadChunk();
                } else {
                    self.library.songs.listing = library;
        ...
        async getLibraryAlbumsFull(force = false, index) {
            ...
            function downloadChunk() {
                ...
                if (downloaded == null) {
                    app.mk.api.library
                        .albums('', params, { includeResponseMeta: !0 })
                        .then((response) => {
                            processChunk(response);
                        });
                } else {
            ...
            function processChunk(response) {
                ...
                if (
                    ...
                    console.log(
                        `downloading next chunk - ${library.length} albums so far`
                    );
                    downloadChunk();
                } else {
                    self.library.albums.listing = library;
        ...
        async getLibraryArtistsFull(force = false, index) {
            ...
            if (localStorage.getItem('libraryArtists') != null) {
                this.library.artists.listing = JSON.parse(
                    localStorage.getItem('libraryArtists')
                );
            ...
            function downloadChunk() {
                ...
                if (downloaded == null) {
                    app.mk.api.library
                        .artists('', params, { includeResponseMeta: !0 })
                        .then((response) => {
                            processChunk(response);
                        });
                } else {
                    downloaded
                        .next('', 'artists', { includeResponseMeta: !0 })
                        .then((response) => {
                            processChunk(response);
                        });
                }
            ...
            function processChunk(response) {
                ...
                if (
                    ...
                    console.log(
                        `downloading next chunk - ${library.length} artists so far`
                    );
                    downloadChunk();
                } else {
                    self.library.artists.listing = library;
            ...
            try {
                if (app.showingPlaylist.relationships.tracks.data.length > 0) {
                    let time = Math.round(
                        []
                            .concat(
                                ...app.showingPlaylist.relationships.tracks.data
                            )
                            .reduce(
                                (a, { attributes: { durationInMillis } }) =>
                                    a + durationInMillis,
                                0
                            ) / 1000
                    );
                    ...
                    let secs = time % 60;
                    return (
                        app.showingPlaylist.relationships.tracks.data.length +
                        ' tracks, ' +
                        (hours > 0
        ...
        async getRadioStations(attempt = 0) {
            ...
            try {
                this.radio.personal = await this.mkapi(
                    'recentRadioStations',
                    false,
                    '',
                    {
        ...
        loadLyrics() {
            const musicType =
                MusicKit.getInstance().nowPlayingItem != null
                    ? MusicKit.getInstance().nowPlayingItem['type'] ?? ''
                    : '';
        ...
        loadAMLyrics() {
            const songID =
                this.mk.nowPlayingItem != null
                    ? this.mk.nowPlayingItem['_songId'] ?? -1
                    : -1;
        ...
        async loadYTLyrics() {
            const track =
                this.mk.nowPlayingItem != null
                    ? this.mk.nowPlayingItem.title ?? ''
                    : '';
            const artist =
                this.mk.nowPlayingItem != null
                    ? this.mk.nowPlayingItem.artistName ?? ''
                    : '';
            const time =
                this.mk.nowPlayingItem != null
                    ? Math.round(
                          (this.mk.nowPlayingItem.attributes[
                              'durationInMillis'
                          ] ?? -1000) / 1000
                      ) ?? -1
                    : -1;
            ipcRenderer.invoke('getYTLyrics', track, artist).then((result) => {
                ...
                function loadYT(id, lang) {
                    ...
                    req.onload = function () {
                        ...
                        let captionurl1 = res.substring(
                            res.indexOf(
                                `{"playerCaptionsRenderer":{"baseUrl":"`
                            ) + `{"playerCaptionsRenderer":{"baseUrl":"`.length
                        );
                        let captionurl = captionurl1.substring(
                            0,
                            captionurl1.indexOf(`"`)
                        );
        ...
        loadMXM() {
            ...
            function getToken(mode, track, artist, songid, lang, time) {
                if (attempt > 2) {
                    ...
                    attempt = attempt + 1;
                    let url =
                        'https://apic-desktop.musixmatch.com/ws/1.1/token.get?app_id=web-desktop-app-v1.0&t=' +
                        revisedRandId();
                    let req = new XMLHttpRequest();
                    ...
                    req.onload = function () {
                        ...
                        let status2 =
                            jsonResponse['message']['header']['status_code'];
                        if (status2 == 200) {
                            let token =
                                jsonResponse['message']['body']['user_token'] ??
                                '';
                            if (
                                token != '' &&
                                token !=
            ...
            function getMXMSubs(track, artist, token, lang, time) {
                ...
                let timecustom =
                    !time || (time && time < 0)
                        ? ''
                        : `&f_subtitle_length=${time}&q_duration=${time}&f_subtitle_length_max_deviation=40`;
                let url =
                ...
                req.onload = function () {
                    ...
                    if (status1 == 200) {
                        ...
                        try {
                            if (
                                jsonResponse['message']['body']['macro_calls'][
                                    'matcher.track.get'
                                ...
                                lrcfile =
                                    jsonResponse['message']['body'][
                                        ...
                                        'subtitle_list'
                                    ][0]['subtitle']['subtitle_body'];

                                try {
                                    lrcrich =
                                        jsonResponse['message']['body'][
                                            'macro_calls'
                                        ]['track.richsync.get']['message'][
                                            'body'
            ...
            function getMXMTrans(id, lang, token) {
                if (lang != 'disabled' && id != '') {
                    ...
                    req2.onload = function () {
                        ...
                        let status2 =
                            jsonResponse2['message']['header']['status_code'];
                        if (status2 == 200) {
                            try {
                                let preTrans = [];
                                let u = app.lyrics;
                                let translation_list =
                                    jsonResponse2['message']['body'][
                                        'translations_list'
                                    ];
        ...
        toMS(str) {
            ...
            let minutes =
                rawTime[2] != null
                    ? hours * 60 + rawTime[2].replace(':', '') * 1
                    : rawTime[1] != null
                    ? rawTime[1].replace(':', '')
                    : 0;
            let seconds = rawTime[3] != null ? rawTime[3] : 0;
            let milliseconds =
                rawTime[4] != null ? rawTime[4].replace('.', '') : 0;
            return parseFloat(
                `${minutes * 60 + seconds * 1}.${milliseconds * 1}`
            );
        },
        parseTTML() {
            this.lyrics = [];
            ...
            if (
                xml
                    ...
                    .getAttribute('itunes:timing') === 'None'
            ) {
                synced = false;
            }
            endTimes.push(0);
            if (synced) {
                for (element of lyricsLines) {
                    ...
                    if (
                        start - endTimes[endTimes.length - 1] > 5 &&
        ...
        getCurrentTime() {
            return parseFloat(
                this.hmsToSecondsOnly(
                    this.parseTime(
                        this.mk.nowPlayingItem.attributes.durationInMillis -
                            app.mk.currentPlaybackTimeRemaining * 1000
                    )
                )
            );
        },
        ...
        playMediaItemById(id, kind, isLibrary, raurl = '') {
            ...
            try {
                if (truekind.includes('artist')) {
                    ...
                    this.mk
                        .setStationQueue({ url: raurl })
                        .then(function (queue) {
                            MusicKit.getInstance().play();
                        });
                } else {
                    this.mk
                        .setQueue({ [truekind]: [id] })
                        .then(function (queue) {
                            MusicKit.getInstance().play();
                        });
                }
        ...
        queueParentandplayChild(parent, childIndex, item) {
            ...
            try {
                if (
                    app.library.songs.listing.length > childIndex &&
                    parent == 'librarysongs'
                ) {
                    console.log(item);
                    if (
                        item &&
                        app.library.songs.listing[childIndex].id != item.id
                    ) {
                        childIndex = app.library.songs.listing.indexOf(item);
                    }

                    let query = app.library.songs.listing.map(
                        (item) => new MusicKit.MediaItem(item)
                    );
                    try {
                        app.mk.stop();
                    } catch (e) {}
                    this.mk.clearQueue().then(function (_) {
                        ...
                        if (childIndex != -1) {
                            ...
                            app.mk
                                .playNext({
                                    [item.attributes.playParams.kind ??
                                    item.type]:
                                        item.attributes.playParams.id ?? item.id
                                })
                                .then(function () {
                                    app.mk.changeToMediaAtIndex(
                                        app.mk.queue._itemIDs.indexOf(
                                            item.id
                                        ) ?? 1
                                    );
                                    app.mk.play();
                    ...
                    if (
                        truekind == 'playlists' &&
                        (id.startsWith('p.') || id.startsWith('pl.u'))
                    ) {
                        app.mk
                            .playNext({
                                [item.attributes.playParams.kind ?? item.type]:
                            ...
                            .then(function () {
                                app.mk.changeToMediaAtIndex(
                                    app.mk.queue._itemIDs.indexOf(item.id) ?? 1
                                );
                                app.mk.play().then(function () {
                                    app.mk.clearQueue().then(function () {
                                        if (
                                            app.showingPlaylist &&
                                            app.showingPlaylist.id == id
                                        ) {
                                            let query =
                                                app.showingPlaylist.relationships.tracks.data.map(
                                                    (item) =>
                        ...
                        this.mk
                            ...
                            .then(function (queue) {
                                if (
                                    item &&
                                    queue._itemIDs[childIndex] != item.id
                                ) {
                                    childIndex = queue._itemIDs.indexOf(
                                        item.id
                                    );
                                }
                                if (childIndex != -1) {
        ...
        async inLibrary(items = []) {
            ...
            for (let item of items) {
                ...
                let id = item.playParams.catalogId
            ...
            let found = this.library.songs.listing.filter((item) => {
                if (item['attributes']) {
                    if (
                        ...
                        item['attributes']['playParams']['catalogId'] == id
                    ) {
                        return item;
                    }
                }
        ...
        getMediaItemArtwork(url, height = 64, width) {
            ...
            let newurl = `${url
                ...
                .replace('{h}', height)
                .replace('{f}', 'webp')
                .replace('{c}', width === 900 ? 'sr' : 'cc')}`;

            if (newurl.includes('900x516')) {
                newurl = newurl
                    .replace('900x516cc', '900x516sr')
                    .replace('900x516bb', '900x516sr');
            }
            return newurl;
        },
        _rgbToRgb(rgb = [0, 0, 0]) {
        ...
        getNowPlayingArtworkBG(size = 600) {
            ...
            let bginterval = setInterval(() => {
                ...
                try {
                    if (
                        this.mk.nowPlayingItem &&
                        this.mk.nowPlayingItem['id'] != this.currentTrackID &&
        ...
        async getRating(item) {
            ...
            let response = await this.mk.api.v3.music(
                `/v1/me/ratings/${type}?platform=web&ids=${id}`
            );
