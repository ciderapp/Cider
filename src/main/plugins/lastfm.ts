...
const  sessionPath = resolve(electron.app.getPath('userData'), 'session.json'),
...
export default class LastFMPlugin {
    ...
    private authenticateFromFile() {
        ...
        this._lastfm.setSessionCredentials(sessionData.name, sessionData.key)
        ...
        fs.stat(sessionPath,  (err : any) => {
            if (err) {
                ...
                this._lastfm.authenticate(this._win.store.store.lastfm.auth_token,  (err: any, session: any) => {
                    ...
                    fs.writeFile(sessionPath, tempData, (err: any) => {
                        if (err)
